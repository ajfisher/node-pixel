    ...
    FIRMATA: {
        initialize: {
            value: function(opts) {
                ...
                var MAX_PIXELS = 256; // based on # bytes available in firmata
                ...
                var color_order = opts.color_order || COLOUR_TYPE.GRB; // default GRB
                ...
                if (strip_definition.length > MAX_STRIPS) {
                    var err = new Error("Maximum number of strips " + MAX_STRIPS + " exceeded");
                    this.emit("error", err);
                    return;
                }
                ...
                if (total_length > MAX_PIXELS) {
                    var err = new Error("Maximum number of pixels " + MAX_PIXELS + " exceeded");
                    this.emit("error", err);
                    return;
                }
