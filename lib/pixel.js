        ...
        initialize: {
            value: function(opts) {
                ...
                if (! global.IS_TEST_MODE) {
                    // don't need to worry about this test if we're using a mock
                    if (firmata.firmware.name !== 'node_pixel_firmata.ino') {
                        throw new Error("Please upload NodePixel Firmata to the board");
                    }
                }
...
Strip.prototype.stripLength = function() {
    ...
    if (! this.dep_warning.stripLength) {
        console.info("INFO: strip.stripLength() is deprecated in favour of strip.length");
...
Pixel.prototype.colour = Pixel.prototype.color = function(color, opts) {
    ...
    if (color) {
        ...
        if(typeof(color) === "object") {
            ...
            pixelcolor = ColorString.get(color) || null;
        }
    } else {
        return pixel.color;
    }
